...
export default function SwapRequestsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* Tabs */}
      <div className="flex items-center gap-2 glass-panel p-1.5 rounded-2xl border border-pink-200 max-w-md">
        <button
          onClick={() => setActiveTab('incoming')}
          className={`flex-1 py-2 rounded-xl text-xs font-extrabold transition-all ${
            activeTab === 'incoming'
              ? 'bg-gradient-to-r from-pink-500 to-purple-600 text-white shadow-md'
              : 'text-purple-800 hover:text-pink-600 font-semibold'
          }`}
        >
      ...
      {/* List Feed */}
      {loading ? (
        <div className="text-center py-16 text-xs text-pink-600 font-semibold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {displayedRequests.map((req) => {
            ...
            return (
              <div key={req._id} className="glass-panel p-6 rounded-3xl border border-pink-200/70 space-y-4">
                ...
                <div className="flex items-center gap-4 text-xs text-purple-900 dark:text-slate-300 font-medium">
                  ...
                  <span className="flex items-center gap-1">
                    <Clock className="w-3.5 h-3.5 text-purple-500" /> {req.preferredDuration}
                  </span>
                </div>
                ...
                {!isSender && req.status === 'Pending' && (
                  <div className="flex items-center gap-3 pt-2">
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-pink-500 via-purple-500 to-sky-400 hover:from-pink-600 hover:to-sky-500 text-white text-xs font-extrabold flex items-center justify-center gap-1 shadow-md transition-all"
                    >
                ...
                {req.status === 'Accepted' && (
                  <button
                    ...
                    className="w-full py-2.5 rounded-xl bg-gradient-to-r from-pink-500 via-purple-500 to-sky-400 hover:from-pink-600 hover:to-sky-500 text-white text-xs font-extrabold flex items-center justify-center gap-2 transition-colors shadow-md"
                  >
